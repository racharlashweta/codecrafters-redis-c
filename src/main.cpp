...
void handle_client(int client_fd) {
    char buffer[8192]; // Large buffer for heavy LRANGE or large SETs
    while (true) {
        ...
        // Process all commands in the current buffer
        while (offset < input.size() && input[offset] == '*') {
            try {
                ...
                if (command == "ping") {
                    ...
                    response = to_bulk_string(parts[1]);
                }
                    ...
                    std::lock_guard<std::mutex> lock(kv_mutex);
                    if (kv_store.count(parts[1]) && kv_store[parts[1]].type == T_STRING) {
                        response = to_bulk_string(kv_store[parts[1]].string_val);
                    } else {
                        response = "$-1\r\n";
                    }
                }
                    ...
                    // Fallback for unknown commands to prevent "no content"
